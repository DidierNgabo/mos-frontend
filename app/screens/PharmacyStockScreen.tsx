...
export default function PharmacyStockScreen() {
  ...
  const {
    ...
    isDeletingPharmacyStock,
  } = useAppSelector((s) => s.pharmacyStock);
  ...
  const [dosageFormFilter, setDosageFormFilter] = useState<string | undefined>(
    undefined,
  );
  ...
  const [selectedStock, setSelectedStock] = useState<PharmacyStock | null>(
    null,
  );
  const [txModalOpen, setTxModalOpen] = useState(false);
  const [stockForTx, setStockForTx] = useState<PharmacyStock | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [stockToDelete, setStockToDelete] = useState<PharmacyStock | null>(
    null,
  );
  ...
  const handleModalSubmit = async (values: any) => {
    try {
      if (modalMode === 'create') {
        ...
        await dispatch(
          updatePharmacyStock({ id: selectedStock.id, data: values }),
        ).unwrap();
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Track and manage medication inventory across outreaches.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-center gap-3 bg-white/50 dark:bg-black/20 backdrop-blur-sm p-4 rounded-2xl border border-border/50 shadow-sm">
        ...
        <div className="w-full sm:max-w-[160px]">
          <Select
            onValueChange={(v) => {
              setOffset(0);
              setDosageFormFilter(v === 'all' ? undefined : v);
            }}
          >
            <SelectTrigger className="h-10 rounded-xl bg-white/50 dark:bg-black/50 border-border">
        ...
        <div className="w-full sm:max-w-[160px]">
          <Select
            onValueChange={(v) => {
              setOffset(0);
              setLowStockFilter(v === 'all' ? undefined : v === 'low');
            }}
          >
            <SelectTrigger className="h-10 rounded-xl bg-white/50 dark:bg-black/50 border-border">
      ...
      <p className="text-xs text-muted-foreground px-1">
        <kbd className="inline-flex items-center rounded border border-border bg-muted px-1 text-[10px] font-medium">
          d
        </kbd>{' '}
        Dispense &nbsp;·&nbsp;
        <kbd className="inline-flex items-center rounded border border-border bg-muted px-1 text-[10px] font-medium">
                  ...
                  >
                    ...
                    <TableCell className="hidden md:table-cell">
                      <div className="text-sm">
                        <p>
                          {stock.dosageForm.charAt(0) +
                            stock.dosageForm.slice(1).toLowerCase()}
                        </p>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        <Can do="update" on="PharmacyStock">
                          ...
                          <QuickTransactionPopover
                            ...
                            open={
                              openPopover?.id === stock.id &&
                              openPopover?.type === 'DISPENSE'
                            }
                            onOpenChange={(o) =>
                              setOpenPopover(
                                o ? { id: stock.id, type: 'DISPENSE' } : null,
                              )
                            }
                            onSuccess={loadData}
                            trigger={
                          ...
                          <QuickTransactionPopover
                            ...
                            open={
                              openPopover?.id === stock.id &&
                              openPopover?.type === 'RESTOCK'
                            }
                            onOpenChange={(o) =>
                              setOpenPopover(
                                o ? { id: stock.id, type: 'RESTOCK' } : null,
                              )
                            }
                            onSuccess={loadData}
                            trigger={
                        ...
                        <DropdownMenu>
                          ...
                          >
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() => handleOpenModal('view', stock)}
                            >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-border/50 bg-muted/20">
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={offset === 0 || isLoadingPharmacyStocks}
              >
                Previous
              </Button>
              <Button
                ...
                disabled={
                  offset + limit >= totalNumItems || isLoadingPharmacyStocks
                }
              >
                Next
              </Button>
